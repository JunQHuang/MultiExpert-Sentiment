...
# def _init_model3(device, *args):
#     texts, labels = load_data('processed_reviews.json')
...
# def _forward_model3(model, input_data, batch_size, test=False):
...
def _init_model3(device, *args):
    ...
    numeric_labels = label_encoder.fit_transform(labels)
    preprocessed_texts = [clean(text) for text in texts]
    X_train, X_test, y_train, y_test = train_test_split(preprocessed_texts, numeric_labels, test_size=0.2, random_state=42)
    
    # 定义类权重，加强对中立态度的识别
    class_weights = {0: 1, 1: 2, 2: 1}  # 假设 0: negative, 1: neutral, 2: positive
...
model = _init_model3('cuda:0')
res = _forward_model3(model, texts, 256)
print(res)
